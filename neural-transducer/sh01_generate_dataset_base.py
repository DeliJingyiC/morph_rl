...
import sklearn
file_path = Path(
    "/users/PAS2062/delijingyic/project/morph/neural-transducer/encoded_df_train.csv"
)
...
for i, row in data['feat'].items():
    ...
    removeinf=data.loc[i, 'lemma'].split(':')
    if '' in removeinf:
        removeinf.remove('')
    # print(removeinf)
    # print(len(removeinf))
    ...
    if len(removeinf) !=1:
        ...
        # print(removeinf[0].split('.')[-1])
        # input()
        if removeinf[0].split('.')[-1] =='Inf':
            continue
        else:
            # if len(data.loc[i, 'lemma'])>600:
            #     continue
            # else:
            new_line.append(data.loc[i, 'lemma'].split(".")[0]+';'+data.loc[i, 'lemma'])
            # print(data.loc[i, 'word'])
            # input()
...
df.to_csv("/users/PAS2062/delijingyic/project/morph/neural-transducer/train_dataset.csv", header=None, sep='\t',index=False)
